...
Two MaskRCNN segmentation models (one per half of the IPD object set) and ten
per-object ResNet regression models were trained on the IPD dataset.
Weights are hosted on [HuggingFace](https://huggingface.co/Ali-Rafiaei/ibpc-pose-estimator):
...
**Two-model segmentation:** The IPD object set was split in half with a separate
MaskRCNN trained on each subset, keeping individual model complexity manageable
while covering all objects.
...
localisation. Contributions here: backbone selection, full training infrastructure
